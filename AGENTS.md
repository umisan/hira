...
1. 新しく開発を行うたびにmainブランチから開発ブランチを作成しPRを作成する
2. 実装を行う前に実装計画を作成しdocsディレクトリの下に新しく開発用ディレクトリを作成し保存する（例: yyyy-mmm-dd-feature-XXX）
3. 作業ログを2で作成した開発ディレクトリに都度保存する

### 開発スタイル

- TDDを採用する
- 単体テストは古典派スタイルで記述する
- 機能開発が終わったあとにリファクタリングを実施しコードを整理する
- 過剰なフォールバックや一般化を行わずシンプルにコードを作成し異常時には素直に例外を発生させる
